...
const CreateKpiPage = ({ departments = [], nextKpiCode = '', users = [] }: CreateKpiPageProps) => {
    ...
    const { register, handleSubmit, control, watch, setValue, formState: { errors } } = useForm<IKpiFormInput>({
        defaultValues: {
            ...
            department: '', 
            kpiType: 'AP',
            dataAcquisition: 'ประมวลผล',
            reportingPeriod: 'รายปี',
            targetDirection: 'up',
            isActive: 'active',
            publishLevel: 'level4'
        }
    ...
    const selectedKpiType = watch('kpiType');
    const selectedDepartment = watch('department');

    useEffect(() => {
        if (selectedKpiType && selectedDepartment) {
            fetch(`/kpis/generate-code?kpi_type=${selectedKpiType}&department_id=${selectedDepartment}`)
                .then(res => res.json())
                .then(data => {
                    setValue('kpiCode', data.code);
                })
                .catch(err => console.error("Error generating KPI Code:", err));
        }
    ...
    return (
        <MainLayout>
            <PageTitle title="Create KPI" subTitle="KPI Management" />
            <div className="mt-4">
                            ...
                            <Card className="shadow-sm">
                                ...
                                <CardBody>
                                    <Row className="mb-4">
                                        <Col md={4}>
                                            <Form.Group>
                                                <Form.Label className="fw-bold d-block text-success mb-2">
                                                    <IconifyIcon icon="tabler:toggle-right" className="me-1 align-middle" />
                                                    สถานะการใช้งาน
                                                </Form.Label>
                                                <div>
                                                    <ButtonGroup>
                                        ...
                                        <Col md={8}>
                                            <Form.Group>
                                                ...
                                                <div>
                                                    <ButtonGroup>
                                                        <ToggleButton id="publish-1" type="radio" variant={watch('publishLevel') === 'level1' ? 'danger' : 'outline-danger'} name="publishLevel" value="level1" checked={watch('publishLevel') === 'level1'} onChange={(e) => setValue('publishLevel', e.currentTarget.value, { shouldValidate: true })}>ผู้ดูแลระบบ</ToggleButton>
                                                        <ToggleButton id="publish-2" type="radio" variant={watch('publishLevel') === 'level2' ? 'warning' : 'outline-warning'} name="publishLevel" value="level2" checked={watch('publishLevel') === 'level2'} onChange={(e) => setValue('publishLevel', e.currentTarget.value, { shouldValidate: true })}>หัวหน้า</ToggleButton>
                                                        <ToggleButton id="publish-3" type="radio" variant={watch('publishLevel') === 'level3' ? 'primary' : 'outline-primary'} name="publishLevel" value="level3" checked={watch('publishLevel') === 'level3'} onChange={(e) => setValue('publishLevel', e.currentTarget.value, { shouldValidate: true })}>เจ้าหน้าที่</ToggleButton>
                                    ...
                                    <Row className="mb-3">
                                        <Col md={12}>
                                            <Form.Group>
                                                ...
                                                <div className="mt-2">
                                                    <ButtonGroup>
                                                        <ToggleButton id="type-ap" type="radio" variant={watch('kpiType') === 'AP' ? 'primary' : 'outline-primary'} name="kpiType" value="AP" checked={watch('kpiType') === 'AP'} onChange={(e) => setValue('kpiType', e.currentTarget.value, { shouldValidate: true })}>AP</ToggleButton>
                                                        <ToggleButton id="type-qmp" type="radio" variant={watch('kpiType') === 'QMP' ? 'primary' : 'outline-primary'} name="kpiType" value="QMP" checked={watch('kpiType') === 'QMP'} onChange={(e) => setValue('kpiType', e.currentTarget.value, { shouldValidate: true })}>QMP</ToggleButton>
                                                        <ToggleButton id="type-qp" type="radio" variant={watch('kpiType') === 'QP' ? 'primary' : 'outline-primary'} name="kpiType" value="QP" checked={watch('kpiType') === 'QP'} onChange={(e) => setValue('kpiType', e.currentTarget.value, { shouldValidate: true })}>QP</ToggleButton>
                                                    </ButtonGroup>
                                                </div>
                                            </Form.Group>
                                        </Col>
